...
if(strcmp($new_password, $verify_password) != 0){
	header("Location: changepasswordform.php?err=1"); //blad: nowe hasla nie zgadzaja sie
	die();
}
else if(strlen($new_password)<8){
	header("Location: changepasswordform.php?err=2"); //blad: za krotkie haslo
	die();
}
if(!$connect->connect_error){
    $result=$connect->query("SELECT * From users$dbprefix WHERE login='$login'");
    if(mysqli_num_rows($result) <= 0){
        header("Location: changepasswordform.php?err=3"); //blad: sesja uszkodzona, uzytkownik nie istnieje. Nieznany blad
        die();
    }
    $row=$result->fetch_assoc();
    if(!password_verify($old_password, $row['password'])) {
        header("Location: changepasswordform.php?err=4"); //blad: stare haslo jest nieprawidlowe
        die();
    } //teraz wszystko ok
    ...
    if($encryption_mode !== '1'){ //jesli haslo konta nie jest kluczem szyfrujacym
        header("Location: panelstat.php"); //sukces, zmieniono haslo.
        die();
    }
    ...
    header("Location: logout.php"); //sukces, zmieniono haslo. Wylogowano.
    die();
}
